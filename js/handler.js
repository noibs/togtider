...
navigator.geolocation.getCurrentPosition(
  (position) => {
    ...
    fetch(
      `https://xmlopen.rejseplanen.dk/bin/rest.exe/trip?originId=${originId}&destId=${destId}&time=${time}&useBus=0`
    )
      ...
      .then((data) => {
        ...
        for (let i = 0; i < trips.length; i++) {
          ...
          let originTime =
            origin.getAttribute('rtTime') || origin.getAttribute('time');
          let destinationTime =
            destination.getAttribute('rtTime') ||
            destination.getAttribute('time');

          let originDelayText = origin.getAttribute('rtTime')
            ? getDelay(
                origin.getAttribute('rtTime'),
                origin.getAttribute('time')
              )
            : '';
          let destinationDelayText = destination.getAttribute('rtTime')
            ? getDelay(
                destination.getAttribute('rtTime'),
                destination.getAttribute('time')
              )
          ...
          tripData.push({
            origin: {
              ...
              time: originTime,
              track: isBus
                ? `<i class="fa-regular fa-bus"></i> Togbus`
                : `Spor: ${origin.getAttribute('rtTrack')}` ||
                  `Spor: ${origin.getAttribute('track')}`,
            ...
            destination: {
              ...
              track: isBus
                ? `<i class="fa-regular fa-bus"></i> Togbus`
                : `Spor: ${destination.getAttribute('rtTrack')}` ||
                  `Spor: ${destination.getAttribute('track')}`,
        ...
        const warningDivSelector =
          window.innerWidth <= 768 ? '.warning-mobile' : '.warning-desktop';
        ...
        tripData.forEach((trip, index) => {
          const tripClass =
            window.innerWidth <= 768 ? 'trip-mobile' : 'trip-desktop';
          const originDelayClass = trip.origin.delayed ? '' : 'transparent';
          const destinationDelayClass = trip.destination.delayed
            ? ''
            : 'transparent';
          ...
          const tripElement = `
                <div class="${tripClass} trip trip-${index + 1}">
                    <div class="title_time">
                        <h2 class="station origin-station">${trip.origin.name.slice(
                          0,
                          -1
                        )}:</h2>
                        <span class="tooltip-container">
                            <span class="delay-indicator ${originDelayClass}">+${
            (typeof trip.origin.delayText === 'string' &&
              trip.origin.delayText.match(/\d+/g) &&
              trip.origin.delayText.match(/\d+/g).join('')) ||
            '0'
          }</span>
                            <i class="fa-regular fa-clock ${originDelayClass}"></i>
                            <span class="tooltip-text">${
                              trip.origin.delayText
                            }</span>
                    ...
                    <div class="title_time">
                        <h2 class="station destination-station">${trip.destination.name.slice(
                          0,
                          -1
                        )}:</h2>
                        <span class="tooltip-container">
                            <span class="delay-indicator ${destinationDelayClass}">+${
            (typeof trip.destination.delayText === 'string' &&
              trip.destination.delayText.match(/\d+/g) &&
              trip.destination.delayText.match(/\d+/g).join('')) ||
            '0'
          }</span>
                            <i class="fa-regular fa-clock ${destinationDelayClass}"></i>
                            <span class="tooltip-text">${
                              trip.destination.delayText
                            }</span>
        ...
        tripData.forEach((trip, index) => {
          ...
          if (destinationName !== destName) {
            ...
            document
              .querySelector(`.trip-${index + 1} .multipleStops`)
...
function getDelay(rtTime, time) {
  ...
  let delayText =
    delay === 1
      ? `Forsinket:<br> ${delay} minut`
      : `Forsinket:<br> ${delay} minutter`;
  return delayText;
}
...
function getData() {
  ...
  return new Promise((resolve, reject) => {
    // Fetch trip data from the Rejseplanen API
    fetch(
      `https://xmlopen.rejseplanen.dk/bin/rest.exe/trip?originId=${originId}&destId=${destId}&useBus=0&time=${time}`
    )
      .then((response) => response.text())
      .then((data) => {
        ...
        trips.forEach((trip, index) => {
          ...
          const originTrackText = isBus
            ? `<i class="fa-regular fa-bus"></i> Togbus`
            : `Spor: ${origin.getAttribute('rtTrack')}` ||
              `Spor: ${origin.getAttribute('track')}`;
          const destinationTrackText = isBus
            ? `<i class="fa-regular fa-bus"></i> Togbus`
            : `Spor: ${destination.getAttribute('rtTrack')}` ||
              `Spor: ${destination.getAttribute('track')}`;
          ...
          const tripClass =
            window.innerWidth <= 768 ? 'trip-mobile' : 'trip-desktop';
          const originDelayClass = origin.getAttribute('rtTime')
            ? ''
            : 'transparent';
          const destinationDelayClass = destination.getAttribute('rtTime')
            ? ''
            : 'transparent';
          const originDelayText = origin.getAttribute('rtTime')
            ? getDelay(
                origin.getAttribute('rtTime'),
                origin.getAttribute('time')
              )
            : '';
          const destinationDelayText = destination.getAttribute('rtTime')
            ? getDelay(
                destination.getAttribute('rtTime'),
                destination.getAttribute('time')
              )
          ...
          const tripElement = `
                <div id="trips" class="${tripClass} trip trip-${index + 1}">
                    <div class="title_time">
                        ...
                        <span class="tooltip-container">
                            <span class="delay-indicator ${originDelayClass}">+${
            (typeof originDelayText === 'string' &&
              originDelayText.match(/\d+/g) &&
              originDelayText.match(/\d+/g).join('')) ||
            '0'
          }</span>
                            <i class="fa-regular fa-clock ${originDelayClass}"></i>
                    ...
                    <div class="title_time">
                        ...
                        <span class="tooltip-container">
                            <span class="delay-indicator ${destinationDelayClass}">+${
            (typeof destinationDelayText === 'string' &&
              destinationDelayText.match(/\d+/g) &&
              destinationDelayText.match(/\d+/g).join('')) ||
            '0'
          }</span>
                            <i class="fa-regular fa-clock ${destinationDelayClass}"></i>
...
function swap() {
  return new Promise((resolve, reject) => {
    try {
      let swappedValues = swapOriginAndDestination(
        originId,
        originName,
        destId,
        destName,
...
function changeTheme() {
  ...
  localStorage.setItem(
    'theme',
    element.classList.contains('darkmode') ? 'darkmode' : 'lightmode'
  );
}
